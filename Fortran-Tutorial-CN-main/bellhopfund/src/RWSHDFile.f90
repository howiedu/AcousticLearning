...
CONTAINS
  SUBROUTINE ReadHeader( FileName, Title, atten, PlotType )
    ...
    OPEN( UNIT = SHDFile,   FILE = FileName, STATUS = 'OLD', ACCESS = 'DIRECT', FORM = 'UNFORMATTED', RECL = 1, &
         IOSTAT = IOStaT, ACTION = 'READ' )
    IF ( IOStat /= 0 ) CALL ERROUT( 'ReadHeader', 'Unable to open shade file' )

    READ( SHDFile, REC = 1 ) LRecl
    CLOSE( UNIT = SHDFile )
    OPEN(  UNIT = SHDFile,   FILE = FileName, STATUS = 'OLD', ACCESS = 'DIRECT', FORM = 'UNFORMATTED', RECL = 1 * LRecl )
  ...
  SUBROUTINE WriteHeader( FileName, Title, freq0, atten, PlotType )
    ...
    IF ( PlotType( 1 : 2 ) /= 'TL' ) THEN
       ...
       OPEN ( FILE = FileName, UNIT = SHDFile, STATUS = 'REPLACE', ACCESS = 'DIRECT', RECL = 1 * LRecl, FORM = 'UNFORMATTED')
