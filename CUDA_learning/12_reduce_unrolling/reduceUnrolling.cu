// 在CUDA平台上实现数组求和归约操作的各种优化技术
// 展示了不同归一情况下，产生线程分支，导致的计算效率下降
// 以及通过展开循环，减少线程分支，提高计算效率的优化方法
...
{
	// 这一种优化方法就是用几个线程开几个线程，多的线程会去做别的工作，不会出现线程等待
	// 这一部分看看博客，那个最后的优化算法就是这个
	// 其实就是尽可能减少开额外的线程，这样就能加速
	// set thread ID
	unsigned int tid = threadIdx.x;
